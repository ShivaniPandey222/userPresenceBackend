...
//    @PostMapping("/notify")
//    public ResponseEntity<Map<String, String>> notifyContentUpdate(@RequestBody Map<String, String> request) {
...
//        return ResponseEntity.ok(Map.of("message", "Users notified about content update"));
//
//    }
//
...
public class NoteController {
    ...
    public ResponseEntity<Map<String, String>> getNote(@PathVariable UUID noteId) {
        ...
        if (noteOptional.isPresent()) {
            Note note = noteOptional.get();
            return ResponseEntity.ok(Map.of("title", note.getTitle(), "content", note.getContent()));
        } else {
            return ResponseEntity.ok(Map.of("title", "Note " + noteId.toString(), "content", "No content available."));
        }
    }
    ...
    public ResponseEntity<List<Map<String, String>>> getAllNotes() {
        List<Map<String, String>> noteList = new ArrayList<>();
        noteService.getAllNotes().forEach(note -> noteList.add(Map.of("id", note.getId().toString(), "title", note.getTitle(), "content", note.getContent())));
        return ResponseEntity.ok(noteList);
    }
